...
import { getAddr, clearForm, adAttendance, adLog, getIndiv} from '../../actions/form_actions';
...
class AddAttend extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Added Individual Attendance successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            console.log(dataToSubmit);
            this.props.dispatch(adAttendance(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Added Individual Attendance",dataToSubmit,"/forms/attendnace",'add')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
    ...
    render(){
    const title = "Individual Attendance";
...
export default connect(mapStateToProps)(AddAttend);