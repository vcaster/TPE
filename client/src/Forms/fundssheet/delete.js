...
class DeleteActvityLog extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Individual Weekly Activity Funds Accounting Sheet successfully, You will be redirected shortly.'
              })
        }
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateFundsSheet(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Deleted Individual Weekly Activity Funds Accounting Sheet",dataToSubmit,"/fund_sheet_view/"+dataToSubmit._id,'delete')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
