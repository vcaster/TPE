...
import { getUsrs, clearForm, adAppoint, adLog, getIndiv, sendSms } from '../../actions/form_actions';
...
class AddAppoint extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Added New Appointment successfully, You will be redirected shortly'
              })
        }
        ...
        if(formIsValid){
            ...
            this.props.dispatch(sendSms(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Added New Appointment",dataToSubmit,"/forms/appointment",'add')).then(response =>{});
                        this.props.dispatch(adAppoint(dataToSubmit)).then(response =>{
...
<div className="bootstrap-timepicker">
  <div className="form-group">
    ...
    <div className="input-group date" id="timepicker" data-target-input="nearest">
      <input type="datetime" className="form-control datetimepicker-input" data-target="#timepicker" />
...
export default connect(mapStateToProps)(AddAppoint);