...
import { getIndiv, clearForm, adOver, adLog} from '../../actions/form_actions';
...
class AddOvernight extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Added Overnight Log successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            console.log(dataToSubmit);
            this.props.dispatch(adOver(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Added Overnight Log",dataToSubmit,"/forms/overnight",'add')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
                    ...
                    setTimeout(()=>{
                    this.props.history.push('/forms/overnight_add')
                    },3000)
...
export default connect(mapStateToProps)(AddOvernight);