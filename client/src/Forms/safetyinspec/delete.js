...
import { getAddr, adLog, clearForm, updateSafetyInspec, getSafetyInspecId } from '../../actions/form_actions';
...
class DeleteSafetyInspec extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Daily Progress Note successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateSafetyInspec(dataToSubmit)).then(response =>{
                this.props.dispatch(adLog(this.props.user.userData._id,"Deleted Safety Inspection",dataToSubmit,"/safety_inspection_view/"+dataToSubmit._id,'delete')).then(response =>{
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getSafetyInspecId(id)).then(response=>{
            ...
            const newFormdata  = populateFields(this.state.formdata,this.props.form.form, 'safety_inspection');
            this.setState({
            formdata: newFormdata,
            loading:false
                })
...
export default connect(mapStateToProps)(DeleteSafetyInspec);