...
import { readHouseMeeting, getHouseMeetingId, adLog } from '../../actions/form_actions';
    ...
    createPdf = (data) => {
        ...
        axios.get(`${FORM_SERVER}/house_meeting_fetch?createdAt=${data.createdAt}&name=${data.address.name}`, { responseType: 'blob'})
            .then((res) => {
                ...
                saveAs(pdfBlob, full+data.address.name+'_house_meeting.pdf')
            })
    }
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getHouseMeetingId(id)).then(response=>{
            ...
            if(this.props.form.form){
                this.props.dispatch(adLog(this.props.user.userData._id,"Viewed House Meeting",null,"/house_meeting_view/"+id,'view')).then(response =>{});
                this.setState({
                loading: false
                    })
            this.downloadIntoServer(this.props.form.form)
            this.props.dispatch(readHouseMeeting(this.props.form.form._id));
            };
        });
                            ...
                            <div className="invoice p-3 mb-3">
                                ...
                                <div>
                                    ...
                                    <div id="page-wrap">
                                        ...
                                        <textarea id="header2" defaultValue={"HOUSE MEETING"} />
                                        <div id="identity">
                                        </div>
                                        <div style={{ clear: 'both' }} />
                                        <table id="meta3">
                                            <tbody><tr>
                                                <td colSpan={3} className="meta-head3">IN ATTENDANCE</td>
                                            </tr>
                                            </tbody>
                                            <tbody><tr>
                                                <td  className="meta-head3">INDIVIDUAL(S)</td>
                                                <td colSpan={2} className="meta-head3">STAFF</td>
                                            </tr>
                                                <tr className="item-row">
                                                    <td><textarea style={{ width: '100%' }} defaultValue={this.props.form.form.individual} /></td>
                                                    <td colSpan={3}><textarea style={{ width: '100%' }} defaultValue={this.props.form.form.staff} /></td>
                                                </tr>
                                            </tbody>
                                            <tbody><tr>
                                                <td colSpan={2} className="meta-head3">MEETING NOTES</td>
                                            </tr>
                                                <tr className="item-row">
                                                    <td colSpan={3}><textarea style={{ width: '100%', height: 300, float: 'left' }} defaultValue={this.props.form.form.meetingnotes} /></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                        <table id="meta3">
                                            <tbody><tr>
                                                ...
                                                <td><img className={styles.sigImage} src={this.props.form.form.staffsignimg} /></td>
                                                <td className="meta-head3">DATE</td>
                                                <td><textarea id="date" defaultValue={moment(this.props.form.form.createdAt).format('LLLL')} /></td>
                                            </tr>
                                                <tr>
                                                    <td className="meta-head3">INDIVIDUAL SIGNATURE</td>
                                                    <td><img className={styles.sigImage} src={this.props.form.form.indivsignimg1} /></td>
                                                    <td className="meta-head3">DATE</td>
                                                    <td><textarea id="date" defaultValue={moment(this.props.form.form.createdAt).format('LLLL')} /></td>
                                                </tr>
                                                <tr>
                                                    <td className="meta-head3">INDIVIDUAL SIGNATURE</td>
                                                    <td><img className={styles.sigImage} src={this.props.form.form.indivsignimg2} /></td>
                                                    <td className="meta-head3">DATE</td>
                                                    <td><textarea id="date" defaultValue={moment(this.props.form.form.createdAt).format('LLLL')} /></td>
                                                </tr>
                                                <tr>
                                                    <td className="meta-head3">INDIVIDUAL SIGNATURE</td>
                                                    <td><img className={styles.sigImage} src={this.props.form.form.indivsignimg3} /></td>
                                                    <td className="meta-head3">DATE</td>
                                                    <td><textarea id="date" defaultValue={moment(this.props.form.form.createdAt).format('LLLL')} /></td>
                                                </tr>
                                            </tbody>
                                        </table><table id="meta3">
                                            <tbody><tr>
                                                <td className="meta-head3">RESIDENTIAL SUPERVISOR SIGNATURE</td>
                                                <td><img className={styles.sigImage} src={this.props.form.form.supervisorsignimg} /></td>
                                                <td className="meta-head3">DATE</td>
                                                <td><textarea id="date" defaultValue={moment(this.props.form.form.createdAt).format('LLLL')} /></td>
                                            </tr>
                                                <tr>
                                                    <td className="meta-head3">PROGRAM MANAGER SIGNATURE</td>
                                                    <td><img className={styles.sigImage} src={this.props.form.form.managersignimg} /></td>
                                                    <td className="meta-head3">DATE</td>
                                                    <td><textarea id="date" defaultValue={moment(this.props.form.form.createdAt).format('LLLL')} /></td>
                                                </tr>
                                            </tbody>
