...
import { getAddr, clearForm, adStaffDescB, adLog } from '../../actions/form_actions';
...
class AddStaffDescB extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Added Staff Job Description 3pm - 11pm successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            console.log(dataToSubmit);
            this.props.dispatch(adStaffDescB(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Added Job Description 3pm - 11pm",dataToSubmit,"/forms/activity_log",'add')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
...
export default connect(mapStateToProps)(AddStaffDescB);