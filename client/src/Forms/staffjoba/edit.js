...
import { getAddr, clearForm, updateStaffDescA, getStaffDescAId, adLog } from '../../actions/form_actions';
...
class EditStaffDescA extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Edited Job Description 11pm - 9am successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateStaffDescA(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Modified Job Description 11pm - 9am",dataToSubmit,"/staff_desc_a_view/"+dataToSubmit._id,'edit')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getStaffDescAId(id)).then(response=>{
            ...
            const newFormdata  = populateFields(this.state.formdata,this.props.form.form, 'staff_desc_a');
            this.setState({
            formdata: newFormdata,
            loading: false           
                })
...
export default connect(mapStateToProps)(EditStaffDescA);