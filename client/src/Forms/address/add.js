...
import { clearForm, adAddress, adLog } from '../../actions/form_actions';
...
class AddAddress extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Added Address successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            ...
            this.props.dispatch(adAddress(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Added New Unit",dataToSubmit,"/forms/address",'add')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
