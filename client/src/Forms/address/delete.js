...
class DeleteAddress extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Address successfully, You will be redirected shortly.'
              })
        }
        ...
        if(formIsValid){
            this.props.dispatch(updateAddress(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Deleted a Unit",dataToSubmit,"/address_view/"+dataToSubmit._id,'delete')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
...
export default connect(mapStateToProps)(DeleteAddress);