...
import { getIndiv, getActivityLogToView, adLog } from '../../actions/form_actions';
import CollapseCheckbox from '../../components/utils/collapseCheckbox'
import TableBlock from '../../components/utils/table_block_view'
...
class ViewActvityLog extends Component {
    ...
    createPdf = (data) => {
        ...
        axios.post(`${FORM_SERVER}/activity_log_download`, data).then(
            ()=>axios.get(`${FORM_SERVER}/activity_log_fetch?createdAt=${data[0].createdAt}&name=${data[0].address.name}`, { responseType: 'blob'})
            .then((res) => {
                const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
                saveAs(pdfBlob, full+data[0].address.name+'_activity_log.pdf')
            })
        );
    }
    downloadIntoServer = (data) => {
        ...
        const seconds=date.getSeconds()
        const full = month+"-"+day+"-"+year+"T"+hour+"."+minutes+"."+seconds+"_"
        axios.post(`${FORM_SERVER}/activity_log_download`, data);
    }
    ...
    componentDidMount() {
        ...
        this.props.dispatch(adLog(this.props.user.userData._id,"Viewed Activity Log",null,"/activity_log_view",'view')).then(response =>{});
        ...
        this.props.dispatch(getActivityLogToView(
            this.state.skip,
            this.state.limit,
            this.state.filters
        )).then(response => {
            if (this.props.form.articles) {
                this.setState({
                    loading: false,
                    indiv: this.props.form.name,
                    addr: this.props.form.addr
                })
                // this.downloadIntoServer(this.props.form.form)
                // this.props.dispatch(readActivityLog(this.props.form.form._id));
            }
        });
        console.log(this.state.filters)
                                ...
                                <div className="col-12">
                                        <div>
                                            ...
                                            <div id="page-wrap">
                                                ...
                                                <textarea id="header2" defaultValue={"ACTIVITY LOG"} />
                                                <div id="identity">
                                                </div>
...
export default connect(mapStateToProps)(ViewActvityLog);