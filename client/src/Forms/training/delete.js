...
class DeleteTraining extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Training successfully, You will be redirected shortly.'
              })
        }
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateTraining(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Deleted Training",dataToSubmit,"/training_view/"+dataToSubmit._id,'edit')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
