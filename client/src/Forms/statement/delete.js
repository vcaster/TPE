...
import { clearForm, updateStatement, getStatementId, adLog } from '../../actions/form_actions';
...
class DeleteStatement extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Employee Statement successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            this.props.dispatch(updateStatement(dataToSubmit)).then(response =>{
                this.props.dispatch(adLog(this.props.user.userData._id,"Deleted Employee Statement",dataToSubmit,"/statement_view/"+dataToSubmit._id,'delete')).then(response =>{
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getStatementId(id)).then(response=>{
            ...
            const newFormdata  = populateFields(this.state.formdata,this.props.form.form, "statement");
            this.setState({
            formdata: newFormdata,
            loading:false
                })
...
export default connect(mapStateToProps)(DeleteStatement);