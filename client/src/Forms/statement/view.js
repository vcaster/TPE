...
import { readStatement, getStatementId, adLog } from '../../actions/form_actions';
...
class ViewStatement extends Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getStatementId(id)).then(response=>{
            
            if(this.props.form.form){
                this.props.dispatch(adLog(this.props.user.userData._id,"Viewed Employee Statement",null,"/house_meeting_view/"+id,'view')).then(response =>{});
                this.setState({
                loading: false
                    })
            // this.downloadIntoServer(this.props.form.form)
            this.props.dispatch(readStatement(this.props.form.form._id));
            };
        });
                                ...
                                <div className="col-12">
                                    ...
                                    <div>
                                        ...
                                        <textarea style={{display: "block", marginLeft: "auto", marginRight: "auto",  width: "100%", textAlign: "center", padding: "20px 0"}} defaultValue={COMPANY_INFO}/>
                                            <textarea id="header2" defaultValue={"EMPLOYEE STATEMENT FORM"} />
                                            <div id="identity">
                                            </div>
                                            <div style={{ clear: 'both' }} />
                                            <table id="meta3">
                                                <tbody><tr>
                                                    <td colSpan={2} className="meta-head3">STATEMENT</td>
                                                </tr>
                                                    <tr className="item-row">
                                                        <td colSpan={3}><textarea style={{ width: '100%', height: 400, float: 'left' }} defaultValue={this.props.form.form.input} /></td>
                                                    </tr>
                                                </tbody>
                                                <tbody><tr>
                                                    <td colSpan={2} className="meta-head3">STAFF NAME</td>
                                                </tr>
                                                    <tr className="item-row">
                                                ...
                                                <tbody><tr>
                                                    <td colSpan className="meta-head3">SIGNATURE</td>
                                                    <td colSpan={2} className="meta-head3">DATE</td>
                                                </tr>
                                                    <tr className="item-row">
                                                        <td><img className={styles.sigImage} src={this.props.form.form.staffsignimg} /></td>
                                                        <td colSpan={3}><textarea style={{ width: '100%' }} defaultValue={moment(this.props.form.form.createdAt).format ("LLLL")} /></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
...
export default connect(mapStateToProps)(ViewStatement);