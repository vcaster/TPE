...
class DeleteIncident extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Incident Report successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateIncident(dataToSubmit)).then(response =>{
                this.props.dispatch(adLog(this.props.user.userData._id,"Deleted Incident Report",dataToSubmit,"/incident_view/"+dataToSubmit._id,'delete')).then(response =>{
...
export default connect(mapStateToProps)(DeleteIncident);