...
class EditDaysProg extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Edited 30 Days Progress Note successfully, You will be redirected shortly.'
              })
        }
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateDaysProg(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Modified 30 Days Progress Note",dataToSubmit,"/days_prog_view/"+dataToSubmit._id,'edit')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getDaysProgId(id)).then(response=>{
            ...
            const newFormdata  = populateFields(this.state.formdata,this.props.form.form, 'days_prog');
            this.setState({
            formdata: newFormdata,
            loading: false           
                })
...
export default connect(mapStateToProps)(EditDaysProg);