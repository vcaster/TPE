...
import { clearForm, updateMessage, getMessageId, adLog } from '../../actions/form_actions';
...
class DeleteMessage extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Deleted Message, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateMessage(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Deleted a Message",dataToSubmit,"/message_delete/"+dataToSubmit._id,"delete")).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
    ...
    componentDidMount() {
        ...
        // console.log(id)
        this.props.dispatch(getMessageId(id)).then(response=>{
            // if(!this.props.form.Dailyprognoteid){
                //console.log(id+"product found ")
            //     this.props.history.push('/')
            // }
            console.log(this.props.user.form);
            const newFormdata  = populateFields(this.state.formdata,this.props.user.form,'Message');
            this.setState({
            formdata: newFormdata,
            loading: false           
                })
                ...
                <div className="container-fluid">
                    <div className="row">
                        ...
                        <form onSubmit={(event)=>this.submitForm(event)}>
                            ...
                            <div className="editor" style={{border: 'solid', padding: '20px'}}>
                            {parse(this.state.formdata.message.value)}
                            </div>
                            <FormField
                                id={'link'}
