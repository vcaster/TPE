...
import { getAddr, clearForm, updateIndiv, updateIndivEnc, getIndivId } from '../actions/user_actions';
...
class EditUser extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Edited Individual Profile successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            ...
            this.props.dispatch(updateIndiv(dataToSubmit)).then(response =>{
                this.props.dispatch(adLog(this.props.user.userData._id,"Modified an Individual",dataToSubmit,"/individual_view/"+dataToSubmit._id,'edit')).then(response =>{
            ...
            axios.post('/api/users/individual_update_enc', { ...dataToSubmit });
        });
    ...
    submitFormEnc = (event) => {
        ...
        if(formIsValid){
            console.log(dataToSubmit);
            this.props.dispatch(updateIndivEnc(dataToSubmit)).then(response =>{
                this.props.dispatch(adLog(this.props.user.userData._id,"Encrypted an Individual",dataToSubmit,"/individual_view/"+dataToSubmit._id,'edit')).then(response =>{
                if(response.payload.success){
                    this.setState({
                        formError: false,
                        formSuccess: true
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getIndivId(id)).then(response=>{
            ...
            const newFormdata  = populateFields(this.state.formdata,this.props.user.form, 'indivi');
            console.log(newFormdata)
            this.setState({
            formdata: newFormdata,
            loading: false           
                    ...
                    <div className="row">
                        ...
                        :null}
                            ...
                            <div className="row">
                                ...
                                <div className="col-4">
                                <button className="btn btn-block bg-gradient-danger" type="button" onClick={(event)=> this.submitFormEnc(event)}>
                                    Encrypt {title}
                                </button>
                                </div>
