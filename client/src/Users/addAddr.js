...
import { clearForm, adAddrAss } from '../actions/user_actions';
import Swal from 'sweetalert2'
import withReactContent from 'sweetalert2-react-content'
import { getUsrs, getAddr } from '../actions/user_actions';
...
class AddAddr extends Component {
    ...
    success = () => {
        if (this.state.formSuccess){
            MySwal.fire({
                icon: 'success',
                title: 'Added Address Association successfully, You will be redirected shortly.'
              })
        }
    ...
    submitForm = (event) => {
        ...
        if(formIsValid){
            console.log(dataToSubmit);
            this.props.dispatch(adAddrAss(dataToSubmit)).then(response =>{
                if(response.payload.success){
                    this.props.dispatch(adLog(this.props.user.userData._id,"Added Address Association",dataToSubmit,"/view_user/"+dataToSubmit.name,'add')).then(response =>{});
                    this.setState({
                        formError: false,
                        formSuccess: true
                    });
    ...
    render(){
    const title = "Address Association";
                ...
                <div className="container-fluid">
                    <div className="row">
                        ...
                        <form onSubmit={(event)=>this.submitForm(event)}>                            
                            ...
                            <FormField
                                id={'addrname'}
                                formdata={this.state.formdata.addrname}
                                change={(element) => this.updateForm(element)}
                            />
...
export default connect(mapStateToProps)(AddAddr);